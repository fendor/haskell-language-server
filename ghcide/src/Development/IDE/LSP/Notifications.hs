...
instance Pretty Log where
  pretty = \case
    ...
    LogWarnNoWatchedFilesSupport -> "Warning: Client does not support watched files. Falling back to OS polling"
...
descriptor recorder plId = (defaultPluginDescriptor plId desc) { pluginNotificationHandlers = mconcat
  ...
  , mkPluginNotificationHandler LSP.SMethod_Initialized $ \ide _ _ _ -> do
      ...
      unless success $
        liftIO $ logWith recorder Debug LogWarnNoWatchedFilesSupport
  ],
